document.addEventListener('DOMContentLoaded', function () {
    ...
    function renderQuiz() {
        ...
        quizData.forEach((data, index) => {
            ...
            for (const [key, value] of Object.entries(data.options)) {
                htmlOutput += `
                    <label class="option">
                        <input type="radio" name="${data.id}" value="${key}" onchange="checkCompletion()">
    ...
    // --- 2. LOGIKA VALIDASI TOMBOL (FITUR BARU) ---
    window.checkCompletion = function () {
        ...
        if (answeredCount === totalQuestions) {
            ...
            // Ubah teks tombol memberitahu sisa soal
    ...
    function startTimer() {
        ...
        timerInterval = setInterval(() => {
            ...
            if (timeRemaining <= 0) {
                ...
                finishQuiz(true); // Paksa selesai walau belum terjawab semua
            }
    ...
    submitBtn.addEventListener('click', function () {
        // Double check (jaga-jaga walau tombol sudah disabled)
        const answeredCount = document.querySelectorAll('input[type="radio"]:checked').length;
    ...
    // --- INITIALIZATION ---
    renderQuiz();
    checkCompletion(); // Jalankan sekali di awal untuk mematikan tombol
    startTimer();
});